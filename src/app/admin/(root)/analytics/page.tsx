...
export default function Analytics() {
  ...
  const [sadacaData, setSadacaData] = useState([
    ...
    <>
      ...
      {isLiabilitiesModelOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-30 flex items-center justify-center">
          <div className="bg-gray-200 rounded-3xl space-y-8 py-4 px-8 text-center relative max-w-md w-full mx-4">
            ...
            <h1 className="text-2xl font-medium">{t("modals.financialLiabilities.title")}</h1>
      ...
      {isSadacaModelOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-30 flex items-center justify-center">
          <div className="bg-gray-200 rounded-3xl py-8 px-8 text-center relative max-w-2xl w-full mx-4 mb-28 md:mb-0">
            ...
            <h1 className="text-3xl mb-4">{t("modals.sadaca.title")}</h1>
            ...
            <div className="bg-white rounded-4xl w-full md:w-4/5 mx-auto">
              <div className="grid grid-cols-2 bg-gray-100 py-3 px-6 rounded-full">
                <div className={`text-${locale == "ar" ? "right" : "left"}`}>{t("modals.sadaca.date")}</div>
                <div className={`text-${locale == "en" ? "right" : "left"}`}>{t("modals.sadaca.amount")}</div>
              </div>

              <div className="max-h-64 overflow-y-auto no-scrollbar">
                {sadacaData.length === 0 ? (
                  <div className="py-8 text-gray-500">
                    {t("modals.sadaca.noEntries")}
                  </div>
                ) : (
                  sadacaData.map((item, index) => (
                    ...
                    >
                      <div className={`text-${locale == "ar" ? "right" : "left"}`}>{item.date}</div>
      ...
      <section className="bg-gray-200 p-3 md:p-5 rounded-4xl mt-5 mb-24 md:mb-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          ...
          />
          <MetricCard
            title={t("metrics.totalNetProfit")}
            value="967 M"
            chart={<MiniBarChart data={miniChartData} />}
          />
          <MetricCard
            title={t("metrics.subscriptionRevenue")}
            value="967 M"
            chart={<MiniBarChart data={miniChartData} />}
          />
          <div className="bg-white rounded-4xl p-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-medium text-gray-600">
                {t("metrics.financialLiabilities")}
              </h3>
              <button
                onClick={handleLiabilitiesModelClose}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-4">
          ...
          <div className="lg:col-span-4 space-y-4">
                ...
                <ChartContainer config={chartConfig}>
                  ...
                  >
                    ...
                    <YAxis
                      ...
                      tickMargin={4}
                      className="text-sm text-gray-500"
                      tickFormatter={(value) => `${value}k ${t("currencySymbol")}`}
                    />
                    <ChartTooltip
                      cursor={false}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-4xl p-6">
                <h3 className="text-lg font-bold mb-4">{t("metrics.averageOrderValue")}</h3>
              ...
              <div className="bg-white rounded-4xl p-6">
                <div className="">
                  <div className="flex justify-between items-center">
                    <h3 className="text-lg font-semibold">{t("metrics.userAnalysis")}</h3>
                  </div>
          ...
          <div className="lg:col-span-2 space-y-4 flex flex-col">
            ...
            <div className="bg-white p-4 rounded-4xl space-y-4 flex-2/5">
              <StatCard label={t("metrics.shippingCosts")} value="1000" />
              <StatCard label={t("metrics.taxes")} value="0" />
              <StatCard label={t("metrics.orders")} value="0" currency />
              <StatCard label={t("metrics.totalRefunds")} value="0" currency />
            </div>

            {/* Total Sadaca */}
            <div className="bg-white rounded-4xl p-6 flex-2/5">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-lg font-semibold">{t("metrics.totalSadaca")}</h3>
                <button
                  onClick={handleSadacaModelClose}
                  className="bg-gray-200 rounded-full p-2 cursor-pointer outline-none border-none"
                >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-lg">{t("metrics.totalDonations")}</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-lg">{t("metrics.dueCharities")}</span>
                  <div className="text-3xl flex items-center justify-center gap-2">
                    <SaudiRiyal size={24} />
                    <span>900</span>
                  </div>
